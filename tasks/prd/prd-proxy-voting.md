...
- [ ] If a proxy is nominated: display the current proxy email with a "Remove proxy" button
- [ ] If no proxy is nominated: display a text input and "Set proxy" button to nominate one
...
- [ ] `PUT /api/admin/lot-owners/{id}/proxy` with body `{ "proxy_email": "<email>" }` — creates or replaces the proxy; returns `LotOwnerOut`
...
- No per-AGM proxy scoping from the lot owner edit screen — nominations are building-level
