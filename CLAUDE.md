...
- **NullPool (no application-level connection pooling)** — `database.py` uses SQLAlchemy's `NullPool`. Each request acquires a direct connection, uses it, and releases it immediately. Neon's PgBouncer handles multiplexing at the infrastructure level. Neon's 115-connection limit supports up to 115 concurrent Lambda requests. The `DB_POOL_SIZE`, `DB_MAX_OVERFLOW`, and `DB_POOL_TIMEOUT` Vercel env vars are no longer read by the app and can be removed.
